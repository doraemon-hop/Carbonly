...
export const LoginPage = () => {
  ...
  const { login, loginWithGoogle, loginAsDemoCitizen, loginAsDemoMerchant, isFirebaseConfigured } = useAuth();
  ...
  const handleCitizenDemo = async () => {
    ...
    addToast({ title: 'Demo Citizen Mode Active', message: 'Logged in as Geetika Soni (Rank #3)', type: 'info' });
    navigate('/dashboard');
  };

  const handleMerchantDemo = async () => {
    await loginAsDemoMerchant();
    addToast({ title: 'Merchant Mode Active', message: 'Logged in as EarthCraft Studio', type: 'info' });
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
        <div className="bg-white py-8 px-6 shadow-xl shadow-slate-200/50 rounded-3xl border border-slate-200/80 sm:px-10">
          ...
          <div className="mb-6 p-3.5 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border border-emerald-200">
            <div className="text-[11px] font-bold text-emerald-800 flex items-center gap-1 mb-2 uppercase tracking-wide">
              <Sparkles className="w-3.5 h-3.5 text-emerald-600" />
              <span>Instant Hackathon Judge Access:</span>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={handleCitizenDemo}
                className="w-full py-2 px-3 rounded-xl bg-white hover:bg-emerald-100/60 border border-emerald-300 text-emerald-900 text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-sm"
              >
              ...
              <button
                ...
                className="w-full py-2 px-3 rounded-xl bg-white hover:bg-amber-100/60 border border-amber-300 text-amber-900 text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-sm"
              >
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 border border-slate-300 rounded-xl shadow-sm bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 transition"
          >
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            >
              <span>{loading ? 'Signing in...' : 'Sign in to Carbonly'}</span>
